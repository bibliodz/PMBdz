...
// +-------------------------------------------------+
// $Id: procs.inc.php,v 1.22 2009-05-16 11:11:53 dbellamy Exp $
...
function show_procs($dbh) {
	...
	for($i=0;$i<$nbr;$i++) {
		...
		if (array_search ($PMBuserid, $rqt_autorisation)!==FALSE || $PMBuserid == 1) {
			...
			$parity += 1;
	        	$tr_javascript=" onmouseover=\"this.className='surbrillance'\" onmouseout=\"this.className='$pair_impair'\" onmousedown=\"document.location='./catalog.php?categ=caddie&sub=gestion&quoi=procs&action=modif&id=$row[0]';\" ";
        		print "<tr class='$pair_impair' $tr_javascript style='cursor: pointer'>
        		<td width='10'>
        		";
				if ($row[1]!="ACTION") print "	<input class='bouton' type='button' value=' $msg[procs_options_tester_requete] ' onClick=\"document.location='./catalog.php?categ=caddie&sub=gestion&quoi=procs&action=execute&id=$row[0]'\" />"; else print "&nbsp;";
				print pmb_bidi("
					</td>
					<td width='80'>
						$row[1]
					...
					<td>");
				if (preg_match_all("|!!(.*)!!|U",$row[3],$query_parameters)) print "<a href='catalog.php?categ=caddie&sub=gestion&quoi=procs&action=configure&id_query=".$row[0]."'>$msg[procs_options_config_param]</a>";
				print "</td>" ;
