...
// +-------------------------------------------------+
// $Id: cms_document.class.php,v 1.3 2014-02-24 14:52:17 arenou Exp $
			...
			foreach($this->used as $type => $used){
				...
				switch($type){
					case "article" :
						...
						break;
					case "section" :
						$query = "select id_section as id,section_title as title from cms_setions where id_section in (".implode(",",$used).") order by 2";
				...
				if($query){
					...
					if(mysql_num_rows($result)){
						while($row = mysql_fetch_object($result)){
							$form.="
						<tr>
							<td><input type='checkbox' value='article_".$row->id."' name='used[]'/></td>
