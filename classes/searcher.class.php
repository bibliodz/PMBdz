...
// +-------------------------------------------------+
// $Id: searcher.class.php,v 1.120 2014-03-07 11:13:19 dgoron Exp $
...
class searcher_title extends searcher {
	...
	function make_first_search() {
		...
		if (!$author_query && !$all_query && !$categ_query) {
		...
		} else if($author_query && $categ_query && !$all_query && !$title_query){
			// Recherche sur l'auteur et la catégorie :
			$aq_auth=new analyse_query(stripslashes($author_query),0,0,1,1);
			if (!$aq_auth->error) {
				$aq_categ=new analyse_query(stripslashes($categ_query),0,0,0,0);
				if (!$aq_categ->error) {
					$members_auth=$aq_auth->get_query_members("authors","concat(author_name,', ',author_rejete)","index_author","author_id");
					if($thesaurus_auto_postage_search && $auto_postage_query)
						$members_categ=$aq_categ->get_query_members("categories","path_word_categ","index_path_word_categ","num_noeud");
					else
						$members_categ=$aq_categ->get_query_members("categories","libelle_categorie","index_categorie","num_noeud");
					$requete_count = "select count(distinct notice_id) from notices ";
					$requete_count.= $acces_j;
					$requete_count.= ", categories, noeuds, notices_categories, authors, responsability ";
					$requete_count.= "where (".$members_categ["where"].") and (".$members_auth["where"].") ";
					...
					$requete = "select notice_id, ".$members_categ["select"]."+".$members_auth["select"]." as pert from notices ";
					$requete.= $acces_j;
					$requete.= ", categories, noeuds, notices_categories, authors, responsability ";
					$requete.= "where (".$members_categ["where"].") and (".$members_auth["where"].") ";
		...
		} else if($title_query && $categ_query && !$all_query && !$author_query){
			// Recherche sur le titre et la catégorie :
			...
			if (!$aq_title->error) {
				...
				if (!$aq_categ->error) {
					...
					$requete_count.= "where (".$members_title["where"].") and (".$members_categ["where"].") ";
					$requete_count.= "and id_noeud= categories.num_noeud and notices_categories.num_noeud=categories.num_noeud and notcateg_notice = notice_id ";
					$requete_count.= $restrict;
						
					$requete = "select distinct notice_id, ".$members_title["select"]."+".$members_categ["select"]." as pert from notices ";
					$requete.= $acces_j;
					$requete.= ", categories, noeuds, notices_categories ";
					$requete.= "where (".$members_title["where"].") and (".$members_categ["where"].") ";
					$requete.= "and id_noeud= categories.num_noeud and notices_categories.num_noeud=categories.num_noeud and notcateg_notice = notice_id ";
					$requete.= $restrict." order by pert desc, index_serie, tnvol, index_sew ";
		...
		} else if($all_query && $categ_query && !$title_query && !$author_query){
			// Recherche sur la catégorie et l'index global :
			$aq_categ=new analyse_query(stripslashes($categ_query),0,0,0,0);
			if (!$aq_categ->error) {
				$aq=new analyse_query(stripslashes($all_query),0,0,1,1);
				$aq_all2=new analyse_query(stripslashes($all_query));
				if (!$aq->error) {
					...
					$requete_count = "select count(distinct notice_id) from notices ";
					$requete_count.= $acces_j;
					$requete_count.= ", notices_global_index, categories, noeuds, notices_categories ";
					$requete_count.= "where (".$members_categ["where"].") and ($where_term) ";
					$requete_count.= "and notices_global_index.num_notice=notices.notice_id and id_noeud= categories.num_noeud and notices_categories.num_noeud=categories.num_noeud and notcateg_notice = notice_id ";
					$requete_count.= $restrict;
					
					$requete = "select distinct notice_id, ".$members_all["select"]."+".$members_categ["select"]." as pert from notices ";
					$requete.= $acces_j;
					$requete.= ", notices_global_index, categories, noeuds, notices_categories where (".$members_categ["where"].") and ($where_term) ";
					$requete.= "and notices_global_index.num_notice=notices.notice_id and id_noeud= categories.num_noeud and notices_categories.num_noeud=categories.num_noeud and notcateg_notice = notice_id ";
					$requete.= $restrict." order by pert desc, index_infos_global "; 
	...
	function make_first_search() {
		...
		if ($search_subject) {
				...
				if (!$aq->error) {
					...
					if($resnum = mysql_query($qry)){
						$this->nb_s = mysql_result($resnum,0,0);
					}
		...
		if ($search_indexint) {
				...
				if (!$aq->error) {
					...
					$this->nb_id=@mysql_result(@mysql_query("select count(distinct indexint_id) from indexint where indexint_name like '".str_replace("*","%",$search_indexint)."'"),0,0);
					if ($this->nb_id) {
						$this->id_query=@mysql_query("select indexint_id from indexint where indexint_name like '".str_replace("*","%",$search_indexint)."' order by indexint_name*1, indexint_name");
						if ($this->nb_id==1) {
