...
function write() {
	...
	if($charset == "utf-8"){
		$token=iconv("UTF-8", "CP1252//TRANSLIT", $_[2]);
	}else{
		$token = $_[2];
	}
...
function write_row() {
    ...
    foreach ($tokens as $dummykey=>$token) {
...
function _cell_to_rowcol($cell) {
    ...
    function _sort_pagebreaks($breaks) {
        ...
        foreach ($breaks as $dummykey=>$break) {
            $hash["$break"]=1;
        }
    ...
    function _encode_password($plaintext) {
        ...
        foreach ($chars as $dummykey=>$char) {
            $password ^= $char;
        }
...
function write_number() {
    ...
    global $charset;
	if($charset == "utf-8"){
		$num=iconv("UTF-8", "CP1252//TRANSLIT", $_[2]);
	}else{
		$num=$_[2];
	}
//!!!
...
function write_string() {
    ...
    global $charset;
	if($charset == "utf-8"){
		$str=iconv("UTF-8", "CP1252//TRANSLIT", $_[2]);
	}else{
    	$str=$_[2];
	}
    $strlen  = strlen($str);
...
function write_blank() {
    ...
    global $charset;
	if($charset == "utf-8"){
		 $xf= $this->_XF($row, $col, iconv("UTF-8", "CP1252//TRANSLIT", $_[2])); # The cell format
	}else{
...
function write_formula() {
    ...
    global $charset;
	if($charset == "utf-8"){
		$formula= iconv("UTF-8", "CP1252//TRANSLIT", $_[2]);# The formula text string
	}else{
...
function write_url() {
	...
	global $charset;
	if($charset == "utf-8"){
		$_[2]= iconv("UTF-8", "CP1252//TRANSLIT", $_[2]);
	}else{
    	$_[2]= $_[2];
	}
