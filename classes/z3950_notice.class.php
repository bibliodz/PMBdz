...
// +-------------------------------------------------+
// $Id: z3950_notice.class.php,v 1.157 2014-01-30 10:02:29 mhoestlandt Exp $
...
class z3950_notice {
	...
	function get_form ($action, $id_notice=0,$retour='link',$article=false) {
		...
		if ($num_rows_bull == 1) {
			...
			$zone_article_form  = str_replace("!!bull_type_new!!",(($num_bull_match)?'':"checked='checked'"),$zone_article_form );
			$zone_article_form  = str_replace("!!bull_type_use_existing!!",(($num_bull_match)?"checked='checked'":''),$zone_article_form );
		} else {
...
//			$alink = '<a target="_blank" href="'.htmlspecialchars($doc_num["a"]).'">'.htmlspecialchars($doc_num["a"]).'</a>';
	...
	function var_to_post($no_download='1'){
		...
		if($this->bibliographic_level == 'a' && $this->hierarchic_level=='2'){
			...
			/*
			 * Pas d'integration sur la date en lot
			 *
			if(!$num_rows_bull){
			...
			*/
			/*
			 * Pas d'integration sur le libelle de periode en lot
			 * 
			if(!$num_rows_bull){
