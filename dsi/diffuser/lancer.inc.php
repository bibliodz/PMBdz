...
// +-------------------------------------------------+
// $Id: lancer.inc.php,v 1.16 2012-06-25 10:30:38 ngantier Exp $
...
for ($iba=0 ; $iba < sizeof($liste_bannette) ; $iba++) {
	...
	switch($suite) {
		...
		case "exporter" :
			...
			break ;
		case "gen_document" :
			$action_diff_aff .= "<script>openPopUp('./print_doc_dsi.php?id_bannette=$bannette->id_bannette', 'Document DSI : $bannette->id_bannette ', 500, 400, -2, -2, 'toolbar=no, dependent=yes, resizable=yes')</script>" ; 
			break ;
	}
}
