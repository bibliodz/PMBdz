...
// +-------------------------------------------------+
// $Id: expl_groupe.inc.php,v 1.35 2012-09-28 13:38:32 dgoron Exp $
...
$sql .= " empr.id_empr, empr.empr_cb, empr.empr_nom, empr.empr_prenom, ";
$sql .= "pret_idexpl, pret_date, pret_retour, ";
$sql .= "expl_id, notice_id, expl_cb, tit1 ";
$sql .= "FROM empr,pret,empr_groupe, groupe,"; 
if ($empr_groupes_localises) {
	if ($empr_location_id=="") $empr_location_id = $deflt2docs_location ;
	if ($empr_location_id!=0) $sql .= "empr as coords_resp_group,";
}
$sql .= "exemplaires LEFT OUTER JOIN notices "; 
$sql .= "ON exemplaires.expl_notice = notices.notice_id "; 
$sql .= "WHERE ";
...
$sql .= "pret.pret_idempr = empr.id_empr AND pret.pret_idexpl = exemplaires.expl_id AND empr_groupe.empr_id = empr.id_empr AND groupe.id_groupe = empr_groupe.groupe_id ";
$sql .= $critere_requete;
...
if ($numero_page > $nbpages_arrondi) {
	echo "<script language=\"javascript\">document.location.replace(\"".$page."?categ=".$categ."&sub=".$sub."&limite_page=".$limite_page."&empr_location_id=".$empr_location_id."\");</script>";
	}
...
if( $nbpages_arrondi != 0 && empty($numero_page)) {
 	$navpage  ='< '.$msg[48].' | <a href="'.$page.'?categ='.$categ.'&sub='.$sub.'&limite_page='.$limite_page.'&numero_page=1';
 	if ($empr_groupes_localises) $navpage .= '&empr_location_id='.$empr_location_id;
 	$navpage .= '">'.$msg[49].' ></a>'; // on passe la variable numero page à 1
