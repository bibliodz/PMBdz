...
// +-------------------------------------------------+
// $Id: fields_empr.inc.php,v 1.84 2014-01-15 15:14:06 dgoron Exp $
...
function aff_query_auth_empr_search($field,&$check_scripts,$varname) {
	...
	$val_dyn=3;
	$ret.="<input type='text'  completion='$completion' autfield='".$field["NAME"]."' class='saisie-50emr' id='f_".$field["NAME"]."'  persofield='".$field["NAME"]."' autfield='".$field["NAME"]."' name='f_".$field["NAME"]."' $readonly value=\"".htmlentities($libelle,ENT_QUOTES,$charset)."\" />\n";
	$ret.="<input type='hidden' id='".$field["NAME"]."' name='".$varname."[]' value=\"".htmlentities($id,ENT_QUOTES,$charset)."\">";
	
	$ret.="<input type='button' class='bouton' value='...' onclick=\"openPopUp('./select.php?what=$what&caller=$caller&p1=".$field["NAME"]."_$i&p2=f_".$field["NAME"]."_$i&perso_id=".$field["ID"]."&custom_prefixe=".$_custom_prefixe_."&dyn=$val_dyn&perso_name=".$field['NAME']
				."&max_field=n_".$field["NAME"]."&field_id=".$field["NAME"]."&field_name_id=f_".$field["NAME"]."&add_field=add_".$field["NAME"]."', 'select_perso_".$field["ID"]
				."', 700, 500, -2, -2,'toolbar=no, dependent=yes, resizable=yes, scrollbars=yes')\" />";
	$ret.="<input name='".$hidden_name."' id='".$hidden_name."'  value='".htmlentities($id,ENT_QUOTES,$charset)."' type='hidden'>";
...
function aff_html_empr($field,&$check_scripts) {
	...
	$check_scripts.= "
	var i = 0;
	while(document.forms[0].elements['".$field[NAME]."['+i+']']){
		if(dijit.byId('".$field[NAME]."_'+i).get('value') && (dijit.byId('".$field[NAME]."_'+i).get('value') != '<br _moz_editor_bogus_node=\"TRUE\" />') && (dijit.byId('".$field[NAME]."_'+i).get('value') != '<br />')) {
			document.forms[0].elements['".$field[NAME]."['+i+']'].value = dijit.byId('".$field[NAME]."_'+i).get('value');
		}
		i++;
	}";
	if ($field[MANDATORY]==1) $check_scripts.="if (document.forms[0].elements[\"".$field[NAME]."[0]\"].value==\"\") return cancel_submit(\"".sprintf($msg["parperso_field_is_needed"],$field[ALIAS])."\");\n";
	return $ret;
}
