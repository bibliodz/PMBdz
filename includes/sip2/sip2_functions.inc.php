...
// +-------------------------------------------------+
// $Id: sip2_functions.inc.php,v 1.17 2014-02-14 09:06:53 ngantier Exp $
...
function _checkin_response_($values) {
	...
	if (!$resultat) {
		...
		*/		
 		if($retour->message_loc ||$retour->message_resa || $retour->message_retard || $retour->message_amende){
			$ret["SCREEN_MESSAGE"][0]=$retour->message_loc." ".$retour->message_resa." ".$retour->message_retard." ".$retour->message_amende;
			//$ok=0;
 		}	
	}
	$ret["OK"]=$ok;
