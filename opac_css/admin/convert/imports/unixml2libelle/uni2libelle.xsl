...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="element">
		<xsl:param name="code"/>
		<xsl:choose>
			<xsl:when test="$corresp/convert/field[@code=$code]">
				<xsl:value-of select="$corresp/convert/field[@code=$code]/@tag"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="concat('f_',$code)"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="sous_element">
		...
		<xsl:choose>			
			...
			<xsl:otherwise>
				<xsl:value-of select="concat('s_',$sous_code)"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
</xsl:stylesheet>