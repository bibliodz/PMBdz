...
// +-------------------------------------------------+
// $Id: analyse_query.class.php,v 1.67 2014-03-18 10:28:24 arenou Exp $
...
//						}
					...
					// on commence...
					$elem_query = "select distinct $field_id from $table_term where ";
					//on applique la restriction si besoin
					if($restrict){
						$elem_query.= $this->get_field_restrict($restrict,$neg_restrict)." and ";
					}
		...
		if(count($notices_ids)){
			...
			if($this->input !== "*"){
				...
				if($with_explnum && $this->input !== "*"){
					...
					$noti = "select distinct explnum_notice as notice_id, ".$noti_members['select']." as pert from explnum where explnum_notice in (".$notices_ids.")";
					$bull = "select distinct num_notice as notice_id, ".$noti_members['select']." as pert from explnum join bulletins on explnum_bulletin = bulletin_id where num_notice in (".$notices_ids.")";
