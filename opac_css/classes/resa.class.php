...
// +-------------------------------------------------+
// $Id: resa.class.php,v 1.16 2014-01-30 11:11:44 abacarisse Exp $
...
if (!defined('RESA_CLASS')) {
	...
	class reservation {
		...
		function reservation($id_empr = 0, $id_notice = 0, $bulletinage = 0, $cb = '') {
			...
			if ($this->id_notice) {
				...
				if (mysql_num_rows($result)) {
					while ($bulletin = mysql_fetch_object($result)) {
		...
		function check_localisation_expl() {
			...
			$query = "select empr_location from empr where id_empr="
					. $this->id_empr;
			...
			if ($this->id_notice)
				$field_expl = " expl_notice=$this->id_notice ";
			else
				$field_expl = " expl_bulletin=$this->id_bulletin ";
		...
		function delete() {
			...
			$result = @mysql_query($query, $dbh);
			// archivage
			$rqt_arch = "UPDATE resa_archive SET resarc_anulee = 1 WHERE resarc_id_empr = '"
					. $this->id_empr . "' and resarc_idnotice = '" . $id_notice
					. "' and	resarc_idbulletin = '" . $id_bulletin . "' ";
			...
			if (!$result) {
				...
				if (mysql_num_rows($result)) {
					...
					if ($total_dispo) {
						...
						$this->message .= $next_empr->empr_nom . " ".$next_empr->empr_prenom . "</strong>. ("
								. $next_empr->empr_cb . ")";
					}
					return TRUE;
				} else {
		...
		function notice_exists() {
			...
			global $msg;
			if ($this->id_notice)
				$query = "select count(1) from notices where notice_id=".$this->id_notice;
			elseif ($this->id_bulletin)
		...
		function resa_exists() {
			global $dbh;
			global $msg;
			$query = "select count(1) from resa where resa_idempr="
					. $this->id_empr;
			if ($this->id_notice)
				$query .= " and resa_idnotice=" . $this->id_notice;
			elseif ($this->id_bulletin)
		...
		function get_resa_cb() {
			...
			while (($resa = mysql_fetch_object($result))) {
				if ($resa->resa_idempr == $this->id_empr)
					break;
				$rank++;
			}
			$this->resa_rank = $rank;
