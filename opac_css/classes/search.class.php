...
// +-------------------------------------------------+
// $Id: search.class.php,v 1.129 2014-02-25 16:06:50 mbertin Exp $
...
//   							print "<br><br>".$main;
    		...
    		switch ($$inter) {
    			case "and":
    				$requete.="select ".$table.".* from $last_table,$table where ".$table.".notice_id=".$last_table.".notice_id group by notice_id";  
 ...
 //   			global $onglet_persopac;
	...
	function remove_forbidden_fields() {
		...
		foreach($search as $k=>$s) {
			if ($this->fixedfields[substr($s,2)]['UNIMARCFIELD']!='FORBIDDEN') {
				$saved_search[$k]=$s;
			}
		}
