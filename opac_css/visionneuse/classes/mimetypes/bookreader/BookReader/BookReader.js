...
BookReader.prototype.BRSearchCallback = function(results) {
    ...
    if (0 == results.matches.length) {
        var errStr  = "La recherche n'a renvoy\351 aucun r\351sultat.";
        var timeout = 1000;
...
BookReader.prototype.initToolbar = function(mode, ui) {
    ...
    $("#BookReader").append(
          "<div id='BRtoolbar'>"
        +   "<span id='BRtoolbarbuttons'>"
        +     "<form action='javascript:br.search($(\"#textSrch\").val());' id='booksearch'><input type='search' id='textSrch' name='textSrch' val='' placeholder='Chercher dans le document'/><button type='submit' id='btnSrch' name='btnSrch'>Rechercher</button></form>"
...
BookReader.prototype.blankInfoDiv = function() {
    return $([
        '<div class="BRfloat" id="BRinfo">',
            '<div class="BRfloatHead">A propos du livre',
            ...
            '<div class="BRfloatBody">',
                ...
                '<div class="BRfloatMeta">',
                    '<div class="BRfloatTitle">',
                        '<h2><a target="_blank"/></h2>',
                    '</div>',
                '</div>',
            '</div>',
            '<div class="BRfloatFoot">',
                '<a href="http://openlibrary.org/dev/docs/bookreader" target="_blank">A propos de BookReader</a>',
            '</div>',
        '</div>'].join('\n')
    );
}
...
BookReader.prototype.initUIStrings = function()
{
    ...
    var titles = { '.logo': '', // $$$ update after getting OL record
                   '.zoom_in': 'Zoom avant',
                   '.zoom_out': 'Zoom arri\350re',
                   '.onepg': 'Vue une page',
                   '.twopg': 'Vue deux pages',
                   '.thumb': 'Vue vignettes',
                   '.print': 'Imprimer cette page',
                   '.embed': 'Export du BookReader',
                   '.link': 'Lien vers ce livre (cette page)',
                   '.bookmark': 'Marquer cette page',
                   '.read': 'Lire ce livre',
                   '.share': 'Partager ce livre',
                   '.info': 'A propos du livre',
                   '.full': 'Plein \351cran',
                   '.book_left': 'Tourner la page \340 gauche',
                   '.book_right': 'Tourner la page \340 droite',
                   '.book_up': 'Page up',
                   '.book_down': 'Page down',
                   '.play': 'Lire',
                   '.pause': 'Pause',
                   '.BRdn': 'Afficher/Masquer', // Would have to keep updating on state change to have just "Hide nav bar"
                   '.BRup': 'Afficher/Masquer',
                   '.book_top': 'Premi\350re page',
                   '.book_bottom': 'Derni\350re page'
                  };
    if ('rl' == this.pageProgression) {
        titles['.book_leftmost'] = 'Derni\350re page';
        titles['.book_rightmost'] = 'Premi\350re page';
    } else { // LTR
        titles['.book_leftmost'] = 'Premi\350re page';
        titles['.book_rightmost'] = 'Derni\350re page';
    }
